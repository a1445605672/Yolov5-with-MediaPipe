...
class LoadImages:
    def process_frame(self, img):
        ...
        if results.multi_hand_landmarks:  # 如果有检测到手
            ...
            for hand_idx in range(len(results.multi_hand_landmarks)):
                ...
                bbox = xmin, ymin, boxW + 30, boxH + 30
                # cx, cy = bbox[0] + (bbox[2] // 2), \
                #          bbox[1] + (bbox[3] // 2)
                # print('bounding box is', bbox[0] - 50, bbox[1] - 100, bbox[0] + bbox[2] + 50, bbox[1] + bbox[3] + 100)
            return bbox
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            self.bbox = self.process_frame(img0)
            if not ret_val:
                self.count += 1
                self.cap.release()
...
class LoadStreams:
    ...
    def __init__(self, sources='streams.txt', img_size=640, stride=32, auto=True):
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            self.bbox = self.process_frame(self.imgs[i])
        print('')  # newline
